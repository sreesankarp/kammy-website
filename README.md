# kammy-website

The public marketing site for Kammy — application details, privacy policy,
and terms of service. Hosted on GitHub Pages.

This repo is intentionally separate from Kammy's app/server source, which
lives in a private repository. Nothing here should ever reference or contain
that code.

## Before this goes live

- [ ] Fill in the contact email in `privacy.html` and `terms.html`
- [ ] Fill in the effective/updated date in both legal pages
- [ ] Fill in the legal/business name and governing-law jurisdiction in `terms.html`
- [ ] Have a lawyer review the privacy policy and terms of service
- [ ] Point the Play Console "Privacy Policy URL" field at the published `privacy.html` URL

## Local preview

Open `index.html` directly in a browser, or serve the folder with any static
file server, e.g. `python3 -m http.server`.
